...
int main(void)
{
    ...
    /**
     * --- code ---
     * factorial:                   # offset: 0x00, locals (after): 1
     *          params 1            # offset: 0x00, locals (after): 2
     *          load frame(1)       # offset: 0x02, locals (after): 3
     *          load 1              # offset: 0x07, locals (after): 4
     *          is <=               # offset: 0x0A, locals (after): 3
     *          else <false>        # offset: 0x0B, locals (after): 2
     *          load 1              # offset: 0x10, locals (after): 3
     *          return              # offset: 0x13
     * <false>: load frame(1)       # offset: 0x14, locals (after): 3
     *          load frame(1)       # offset: 0x19, locals (after): 4
     *          load 1              # offset: 0x1E, locals (after): 5
     *          sub                 # offset: 0x21, locals (after): 4
     *          call factorial      # offset: 0x22, locals (after): 4
     *          mul                 # offset: 0x27, locals (after): 3
     *          return              # offset: 0x28
     * 
     * main:                        # offset: 0x29
     *          load 10
    ...
    lstf_bc_function_add_instruction(fact_fun,
            lstf_bc_instruction_load_frameoffset_new(1));
    lstf_bc_function_add_instruction(fact_fun,
    ...
    else1->instruction_ref = lstf_bc_function_add_instruction(fact_fun,
            lstf_bc_instruction_load_frameoffset_new(1));
    lstf_bc_function_add_instruction(fact_fun,
            lstf_bc_instruction_load_frameoffset_new(1));
    lstf_bc_function_add_instruction(fact_fun,
    ...
    if (lstf_bc_program_serialize_to_binary(program, ostream)) {
        ...
        lstf_vm_program *vm_program = lstf_vm_loader_load_from_buffer(ostream->buffer, 
                ostream->buffer_offset, &error);

        if (vm_program) {
            lstf_virtualmachine *vm = lstf_virtualmachine_new(vm_program, false);
