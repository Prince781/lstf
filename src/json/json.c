...
static json_node *json_internal_convert_node_to_pattern(json_node *node)
{
    ...
    switch (node->node_type) {
        ...
        {
            json_object_foreach(node, member, {
                (void)member_name;
        ...
        {
            json_array_foreach(node, element, {
                json_array_set_element(node, iterator_of(element),
                                       json_internal_convert_node_to_pattern(element));
            });
        }   break;
...
static void json_node_destroy(json_node *node)
{
    ...
    if (node->node_type == json_node_type_string) {
        ...
        json_array_foreach(array, element, {
            json_node_unref(element);
            array->elements[iterator_of(element)] = NULL;
        });
                                   ...
                                   string *sb)
{
    ...
    case json_node_type_array:
    {
        ...
        json_array_foreach(array, element, {
            ...
            } else if (iterator_of(element) > 0) {
                string_appendf(sb, " ");
            }
            json_node_build_string(root_node, element, pretty, tabulation + 1, sb);
            if (iterator_of(element) < array->num_elements - 1)
                string_appendf(sb, ",");
        });
    ...
    case json_node_type_object:
    {
        ...
        json_object_foreach(object, member, {
            if (pretty) {
                string_appendf(sb, "\n");
                json_node_build_string_tabulate(sb, tabulation + 1);
            } else if (!iterator_of(member).is_first) {
                string_appendf(sb, " ");
            }
            ...
            if (iterator_next(iterator_of(member)).has_next)
                string_appendf(sb, ",");
        });
...
bool json_node_equal_to(json_node *node1, json_node *node2)
{
    ...
    case json_node_type_array:
    {
        ...
        if (node1->is_pattern || node2->is_pattern) {
            ...
            json_array_foreach(array1, elem, {
                if (!json_node_equal_to(elem,
                                        array2->elements[iterator_of(elem)])) {
                    node1->visiting = false;
                    node2->visiting = false;
                    return false;
                }
    ...
    case json_node_type_object:
    {
        ...
        if (node1->is_pattern || node2->is_pattern) {
            ...
            json_object_foreach(object1, object1_member, {
            ...
            if (!node1->is_pattern) {
                json_object_foreach(object2, object2_member, {
            ...
            json_object_foreach(object1, member, {
...
static json_node *json_node_internal_copy(json_node *node, ptr_hashmap *seen_nodes)
{
    ...
    case json_node_type_array:
        ...
        json_node_internal_copy_flags(node, new_node);
        json_array_foreach(node, element, {
            json_array_add_element(new_node, json_node_copy(element));
        });
        break;
    case json_node_type_object:
