...
json_serializable_impl_as_object(lsp_textdocument, "uri", "content");
                                                                       ...
                                                                       json_node        *property_node)
{
    ...
    } else if (strcmp(property_name, "content") == 0) {
        if (property_node->node_type != json_node_type_string)
            return json_serialization_status_invalid_type;
        doc->content = string_ref(string_new_copy_data(((json_string *)property_node)->value));
    } else {
...
static json_serialization_status lsp_textdocument_serialize_property(const lsp_textdocument *doc,
                                                                     const char             *property_name,
                                                                     json_node             **property_node)
{
    if (strcmp(property_name, "uri") == 0) {
        ...
        *property_node = json_string_new(doc->content->buffer);
    } else {
